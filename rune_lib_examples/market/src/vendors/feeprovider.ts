...
export async function calculateTxBytesFee(vinsLength: number, voutsLength: number, feeRateTier: string, includeChangeOutput: 0 | 1 = 1) {
...
export function calculateTxBytesFeeWithRate(vinsLength: number, voutsLength: number, feeRate: number, includeChangeOutput: 0 | 1 = 1): number {
  const baseTxSize = 10;
  const inSize = 180;
  const outSize = 34;
...
export function getSellerRuneOutputValue(price: number, makerFeeBp: number, prevUtxoValue: number): number {
  return (
    price - // listing price
