...
export namespace BuyerHandler {
  async function doesUtxoContainRunes(utxo: AddressTxsUtxo, service: RPCService): Promise<boolean> {
    ...
    if (utxo.status.confirmed) {
      try {
        return (await service.getRuneByTxIdAndIndex(utxo.txid, utxo.vout)) !== null;
      } catch (err) {
    ...
    for (const input of tx.vin) {
      ...
      try {
        if ((await service.getRuneByTxIdAndIndex(input.txid, input.vout)) !== null) {
          foundRunes = true;
          return foundRunes;
        }
      } catch (err) {
  ...
  export async function selectPaymentUTXOs(
    ...
    for (const utxo of utxos) {
      ...
      if (selectedAmount >= amount + (await calculateTxBytesFee(vinsLength + selectedUtxos.length, voutsLength, feeRateTier))) {
        break;
      }
    }
  ...
  export async function generateUnsignedBuyingPSBTBase64(buyer_state: IRuneListingState, seller_items: IRuneListingState[]) {
    const psbt = new bitcoin.Psbt({ network });
    if (!buyer_state.buyer || !buyer_state.buyer.buyerAddress || !buyer_state.buyer.buyerTokenReceiveAddress) {
    ...
    for (let i = 0; i < seller_items.length; i++) {
      ...
      const { sellerInput, sellerOutput } = await getSellerInputAndOutput(seller);
      ...
      _platform_fee += Math.floor((seller.seller.price * (buyer_state.buyer.takerFeeBp + seller.seller.makerFeeBp)) / 10000);
    ...
    const edict = new Edict(BigInt(seller_items[0].seller.runeItem.id), BigInt(_seller_outputs.length + 2), _seller_total_tokens);
    let rs = new RuneStone([edict], null, false, null, null);
    rs.setTag(RUNE_TAG);
    ...
    const _all_outputs_except_change = [..._seller_outputs, op_return_output, platform_output];
  ...
  async function getSellerInputAndOutput(listing: IRuneListingState) {
    ...
    if (listing.seller.tapInternalKey && !listing.seller.publicKey) {
      ...
      sellerInput.tapInternalKey = toXOnly(tx.toBuffer().constructor(toXOnly(Buffer.from(listing.seller.publicKey, 'hex')), 'hex'));
    }
